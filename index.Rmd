...
library(gbm)
  ...
  plotROC <- function(pred){
    ...
    #dev.new(height=400, width=675, unit = "px")
    par("pty")
    plot(perf, xlab='Tasa de falso positivo', ylab='Tasa de verdadero positivo')
...
Aún cuando la herramienta CARET participa en el momento de seleccionar variables, también se utiliza la información obtenido del análisis de datos.
...
```{r model setup, echo=FALSE, warning=FALSE, message=FALSE}
  train <- dataset
  train$Evento = as.numeric(train$Evento)
  train$TBQ = as.numeric(train$TBQ) 
  train$DLP = as.numeric(train$DLP) 
  train$OBES = as.numeric(train$OBES) 
  train$HTA = as.numeric(train$HTA) 
  
  set.seed(28)
  
  index <- createDataPartition(train$Evento, p=0.75, list=FALSE) 
  
  trainSet <- train[index,]
  testSet <- train[-index,]
  
  predictors <- c("HTA", "DLP", "TBQ", "OBES")
  outcomeName <- 'Evento'
...
plot(varImp(object=model_glm), xlab='')
```

<p class="bottom-space">
</p>

Para el modelo la variable más importante es la hipertensión arterial, seguido del tabaquismo, la dislipemia y la obesidad.
...
El valor de la curva AUROC es de `r format(round(perf@y.values[[1]], 3), nsmall = 3)`
